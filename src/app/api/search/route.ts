...
import { filterResults } from "@/lib/search/filter";
import { searchSearxng } from "@/lib/search/searxng";
import type { SearchRequest, WhitelistMode } from "@/lib/search/types";
...
export async function POST(request: Request) {
  ...
  try {
    const rawResults = await searchSearxng({ query, engines, whitelist });
    const filtered = filterResults({ results: rawResults, blacklist, whitelistMode });
